...
class Calendar(widgets.Box):
    def __init__(self, prefix: str = "calendar", **kwargs):
        self.prefix = prefix
        ...
        self.prev_button = widgets.Button(
            ...
            css_classes=[f"{self.prefix}-prev-month-button"],
        )
        self.next_button = widgets.Button(
            child=widgets.Label(label="▶"),
            on_click=self.next_month,
            css_classes=[f"{self.prefix}-next-month-button"],
        )
        self.month_label = widgets.Label(
            label="", css_classes=[f"{self.prefix}-month-label"]
        )
        self.header = widgets.CenterBox(
            start_widget=self.prev_button,
            center_widget=self.month_label,
            end_widget=self.next_button,
            css_classes=[f"{self.prefix}-calendar-header"],
        )

        self.weekday_row = widgets.Box(
            spacing=5,
            halign="center",
            css_classes=[f"{self.prefix}-calendar-weekdays"],
        )
        for d in ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]:
            lbl = widgets.Label(label=d, css_classes=[f"{self.prefix}-weekday-label"])
        ...
        self.grid = widgets.Box(
            vertical=True, spacing=4, css_classes=[f"{self.prefix}-calendar-grid"]
        )

        super().__init__(
            vertical=True,
            css_classes=[f"{self.prefix}-calendar"],
    ...
    def update_calendar(self):
        ...
        for week in month_days:
            ...
            for day in week:
                if day == 0:
                    lbl = widgets.Label(
                        label="", css_classes=[f"{self.prefix}-day-empty"]
                    )
                else:
                    lbl = widgets.Label(
                        label=str(day), css_classes=[f"{self.prefix}-day-label"]
                    )
                    if (
                        ...
                        lbl.add_css_class(f"{self.prefix}-current-day")
                week_row.append(lbl)
