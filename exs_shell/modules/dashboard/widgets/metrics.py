...
class SystemMonitor(widgets.Box):
    def __init__(self):
        ...
        color_hex = get_hex_color()
        rgb = hex_to_rgb(color_hex)

        self.ram_meter = CircularMeter(color=rgb, label="", radius=90, thickness=22)
        self.cpu_meter = CircularMeter(color=rgb, label="", radius=110, thickness=25)
        self.disk_meter = CircularMeter(color=rgb, label="")
